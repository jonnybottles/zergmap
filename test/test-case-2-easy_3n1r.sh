...
cd ../
echo " "
echo "********** BEGIN TEST CASE 2 EASY 3N1R **********"
echo " "
...
# to dev/null.
./zergmap $(cat test/samp/test-case-2-easy_3n1r.txt)

# Based upon the results, a pass or fail message is sent to stdout.
RESULTS="$?"
if [ "$RESULTS" == "$PASS" ]
then
    echo "Pass: zergmap test-case-2-easy_3n1r."
else
    echo "FAIL: zergmap test-case-2-easy_3n1r."
fi