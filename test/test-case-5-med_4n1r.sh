...
cd ../
echo " "
echo "********** BEGIN TEST CASE 5 MED 4n1r **********"
echo " "
...
# to dev/null.
./zergmap $(cat test/samp/test-case-5-med_4n1r.txt)

# Based upon the results, a pass or fail message is sent to stdout.
RESULTS="$?"
if [ "$RESULTS" == "$PASS" ]
then
    echo "Pass: zergmap test-case-5-med_4n1r."
else
    echo "FAIL: zergmap test-case-5-med_4n1r."
fi