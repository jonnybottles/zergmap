...
cd ../
echo " "
echo "********** BEGIN TEST CASE 1 EASY 3N0R **********"
echo " "
...
# to dev/null.
./zergmap $(cat test/samp/test-case-1-easy_3n0r.txt)

# Based upon the results, a pass or fail message is sent to stdout.
RESULTS="$?"
if [ "$RESULTS" == "$PASS" ]
then
    echo "Pass: zergmap test-case-1-easy_3n0r."
else
    echo "FAIL: zergmap test-case-1-easy_3n0r."
fi