...
cd ../
echo " "
echo "********** BEGIN TEST CASE 6 HARD 6n0r **********"
echo " "
...
# to dev/null.
./zergmap $(cat test/samp/test-case-6-hard_6n0r.txt)

# Based upon the results, a pass or fail message is sent to stdout.
RESULTS="$?"
if [ "$RESULTS" == "$PASS" ]
then
    echo "Pass: zergmap test-case-6-hard_6n0r."
else
    echo "FAIL: zergmap test-case-6-hard_6n0r."
fi